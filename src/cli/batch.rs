...
mod tests {
    ...
    #[test]
    fn parse_char_defaults_to_full_block() {
        assert_eq!(parse_char(&None).unwrap(), blocks::FULL);
    }

    #[test]
    fn parse_char_resolves_name_alias() {
        let input = Some("shade-light".to_string());
        assert_eq!(parse_char(&input).unwrap(), blocks::SHADE_LIGHT);
    }

    #[test]
    fn parse_char_resolves_alt_alias() {
        let input = Some("block".to_string());
        assert_eq!(parse_char(&input).unwrap(), blocks::FULL);
    }

    #[test]
    fn parse_char_raw_char_passthrough() {
        let input = Some("░".to_string());
        assert_eq!(parse_char(&input).unwrap(), '░');
    }

    #[test]
    fn parse_char_unknown_returns_error() {
        let input = Some("nonexistent".to_string());
        assert!(parse_char(&input).is_err());
    }

    #[test]
    fn parse_char_empty_string_defaults() {
